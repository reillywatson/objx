...
func (m Map) Remove(selector string) Map {
	access(m, selector, nil, false, true)
	return m
}
...
// appropriate action.
func access(current interface{}, selector string, value interface{}, isSet, isRemove bool) interface{} {
	...
	if nextSel != "" {
		current = access(current, nextSel, value, isSet, isRemove)
	}
	return current
}
